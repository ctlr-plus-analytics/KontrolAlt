-- Fix Substack URL regex in the eligibility trigger to allow dots in handles
-- (e.g. @first.last). The previous regex [A-Za-z0-9_-] excluded dots, causing
-- channels whose handles contain a dot to always fail dashboard_url_valid.
...
-- Recompute eligibility for all Substack channels now that the regex is fixed.
UPDATE channels
SET updated_at = NOW()
